...
from .utils.utils import convert_text_to_speech, generate_voice, handle_conversation_loop,response_for_gpt
    ...
    if request.method == 'POST':
        try:
            ...
            if serializer.is_valid():
                ...
                gather = Gather(
                input="speech dtmf",
                action=f"https://ee12-203-189-248-87.ngrok-free.app/api/ai/process-gather/",
                method="POST",
                timeout=5,
                speechTimeout="1",
                ...
                gather.play("https://0a3b-203-189-248-87.ngrok-free.app/aisalesagent/audio/test.mp3")
...
def process_gather(request):
    if request.method == 'POST':
        ...
        print(speechResult, "recording_url")
       
        ai_response = asyncio.run(response_for_gpt(speechResult));
        ...
        call_sid = request.POST.get('CallSid')  # If sent by Twilios
        ...
        # try:
        #     audio_url, mimetype = loop.run_until_complete(parse_twilio_response(speechResult))
        ...
        # loop = asyncio.new_event_loop()
        # asyncio.set_event_loop(loop)
        # loop.run_until_complete(handle_conversation_loop(speechResult, call_sid))
        # loop.close()
...
async def parse_twilio_response(recording_url):
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            print("Error parsing XML:", xml_content)
            raise ValueError("Failed to parse Twilio response")
